...
export class BladeRunnerEggs extends Container {
  ...
  private pyramid = new Graphics();
  private mottoText: Text;
  private serialText: Text;
  private elapsed = 0;
  private vkTimer = 0;
  private vkAlpha = 0;
  private centerX = 0;
  private centerY = 0;
  private pyramidBurst = 0;   // countdown for click burst glow
  private onPyramidClick: (() => void) | null = null;
  ...
  setPyramidClickHandler(cb: () => void): void {
  ...
  private drawPyramid(): void {
    ...
    this.pyramid.on('pointertap', () => {
      this.pyramidBurst = 1;
      if (this.onPyramidClick) this.onPyramidClick();
    });
  }
  ...
  update(dt: number): void {
    ...
    if (this.pyramidBurst > 0) {
      ...
      if (this.pyramidBurst < 0) this.pyramidBurst = 0;
    ...
    if (this.vkAlpha > 0) {
      ...
      if (showTime > 58) {
        ...
        if (this.vkAlpha <= 0) this.vkAlpha = 0;
      }
    }
